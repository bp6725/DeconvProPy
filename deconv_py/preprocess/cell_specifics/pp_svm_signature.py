...
class PpSvmSignature(TransformerMixin,BaseEstimator) :
    ...
    def transform(self, data, *_):
        ...
        if _A_from_pipe.deconv.must_contain_genes is not None :
            genes_list_idx = genes_list_idx.union(_A_from_pipe.deconv.must_contain_genes).drop_duplicates()

        filt_A = _A_from_pipe.copy(deep=True).loc[genes_list_idx]
        filt_B = _B.copy(deep=True).loc[genes_list_idx]

        if not self.with_norm:
            try:
                filt_A.deconv.transfer_all_relevant_properties(_A_from_pipe)
        ...
        try:
            norm_filt_A.deconv.transfer_all_relevant_properties(_A_from_pipe)
