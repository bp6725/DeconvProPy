...
_new_cells_to_selected_cluster = {
    'B cells ': 'PB.1079966',
...
class CytofCellCountInfra():
    ...
    def get_cytof_count_per_sample(self , cells_to_selected_cluster=_new_cells_to_selected_cluster, filter_by_version="V1"):
        relvent_cytof_data = self.cytof_data[list(itertools.chain(*list(filter(lambda x: isinstance(x, list), cells_to_selected_cluster.values())))) + list(
            filter(lambda x: isinstance(x, str), cells_to_selected_cluster.values()))]

        cells_over_samples_df = relvent_cytof_data.rename(
            columns=self._build_cluster_to_cell_map(cells_to_selected_cluster))
    ...
    def return_mass_and_cytof_not_none_cells_counts(self, mass_results_df, profile_to_cytof_cell_map = None, filter_by_version="V1", is_mean_agg = False):
        ...
        if self._is_cells_alredy_in_cytof_format(mass_results_df,_profilte_to_cytof_cell_map) :
            profile_to_cytof_cell_map = {cell:cell for cell in mass_results_df.index}

        if profile_to_cytof_cell_map is None :
            profile_to_cytof_cell_map = _profilte_to_cytof_cell_map

        if len([cm for cm in mass_results_df.index if cm in profile_to_cytof_cell_map]) == 0 :
            new_profilte_to_cytof_cell_map = {}

            for cell_from_profile in mass_results_df.index :
                corr_cell_in_map = [c for c in profile_to_cytof_cell_map if c in cell_from_profile]
                if len(corr_cell_in_map) == 0 :
                    new_profilte_to_cytof_cell_map[cell_from_profile] = None
                else :
                    new_profilte_to_cytof_cell_map[cell_from_profile] = profile_to_cytof_cell_map[corr_cell_in_map[0]]

            profile_to_cytof_cell_map = new_profilte_to_cytof_cell_map


        not_none_profile_cells = [profile_cell for profile_cell, cytof_cell in profile_to_cytof_cell_map.items() if
        ...
        mass_results_not_none_df["cytof_cell"] = mass_results_not_none_df.index.map(
            lambda x: profile_to_cytof_cell_map[x])
    ...
    def _build_cluster_to_cell_map(self,cells_to_selected_cluster):
        mapping = {}
        for cell, cluster in cells_to_selected_cluster.items():
            if isinstance(cluster, str):
                mapping[cluster] = cell
            if isinstance(cluster, list):
                for _clus in cluster:
                    mapping[_clus] = cell
        return mapping
