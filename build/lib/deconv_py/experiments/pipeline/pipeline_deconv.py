...
from deconv_py.infras.deconv_data_frame import DecovAccessor
...
class PipelineDeconv():
    ...
    def run_cytof_pipeline(self,A,B,per_cell_analysis = False):
        '''
        ...
        with tqdm(total=len(self._static_configurations)) as pbar:
            ...
            for pip in self._pipeline_gen:
                ...
                _func = partial(self._run_pipe_predict,A,B,pip)
                result, X = self._run_or_load(pip_step_str,_func)
                ...
                for measure in self._pipeline_measure_gen:
                    if per_cell_analysis :
                        ...
                        _pip_summary = self._run_or_load(measure_per_cell_str,_measure_per_cell_func)
                    else :
                        measure_str = f"{str(measure.steps)}_{pip_step_str}"
                        _measure_func = partial(self._measure, result, X, measure)
                        _pip_summary = self._run_or_load(measure_str, _measure_func)
    ...
    def _run_or_load(self,name_str,function):
    ...
    def _run_pipe_predict(self,A,B,pip) :
        is_with_label_prop = pip.named_steps['Cytof_X_Building'].with_label_prop
